    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-8 md:px-16 lg:px-24 xl:px-32'>
        ...
        </div>
      {
        roomsDummyData.map((room) => (
          <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
            <img onClick={()=> {navigte(`/rooms/${room._id}`); scrollTo(0,0)}} src={room.images[0]} alt='hotel-room' title='View Room Details' className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'/>
            <div className='md:w-1/2 flex flex-col gap-2'>
              <p className='text-gray-500'>{room.hotel.city}</p>
              <p className='text-gray-800 text-3xl font-playfair cursor-pointer' onClick={()=> {navigte(`/rooms/${room._id}`); scrollTo(0,0)}}>{room.hotel.name}</p>
      ...
      <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300 ${openFilters && 'border-b'}`}>
        ...
        <div className='text-xs cursor-pointer'>
          ...
          <span className='hidden lg:block'>CLEAR</span></div>
      </div>
